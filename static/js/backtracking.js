...
function dibujar_universo(matriz) {
  ...
  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      ...
      if (j === matriz.origen[0] && i === matriz.origen[1]) {
        celda.innerHTML = "🛸";
      } else if (j === matriz.destino[0] && i === matriz.destino[1]) {
        celda.innerHTML = "🌎";
      } else if (esAgujeroNegro(j, i, matriz.agujerosNegros)) {
        ...
        celda.innerHTML = "⚡";
      } else if (esCeldaCargaRequerida(j, i, matriz.celdasCargaRequerida)) {
        celda.classList.add("carga-requerida");
        celda.innerHTML = "🔋";
      }
...
function esAgujeroGusano(x, y, agujerosGusano) {
  return agujerosGusano.some(
    (agujero) => agujero[0][0] === x && agujero[0][1] === y
  );
}

function esEstrellaGigante(x, y, estrellasGigantes) {
  return estrellasGigantes.some(
    (estrella) => estrella[0] === x && estrella[1] === y
  );
}
...
function esCeldaCargaRequerida(x, y, celdasCargaRequerida) {
  return celdasCargaRequerida.some(
    (celda) => celda[0][0] === x && celda[0][1] === y
  );
}
